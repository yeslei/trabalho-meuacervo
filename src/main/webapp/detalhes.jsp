...
<main class="container">
    ...
    <c:if test="${param.sucesso == 'avaliado'}">
        <div class="alert alert-success">AvaliaÃ§Ã£o salva com sucesso!</div>
    </c:if>
    ...
    <%-- ====================================================================
         Bloco principal: capa + info + aÃ§Ãµes
         ==================================================================== --%>
            ...
            <div class="detalhes-rating">
                <span class="stars">
                    <c:forEach begin="1" end="5" var="i">
                        <c:choose>
                            <c:when test="${i <= estatistica.mediaArredondada}">â˜…</c:when>
                            <c:otherwise><span class="star-empty">â˜…</span></c:otherwise>
                        </c:choose>
                    </c:forEach>
                </span>
                ...
                <span class="rating-total">
                    (${estatistica.total}
                    <c:choose>
                        <c:when test="${estatistica.total == 1}">avaliaÃ§Ã£o</c:when>
                        <c:otherwise>avaliaÃ§Ãµes</c:otherwise>
                    </c:choose>)
                </span>
            </div>
            ...
            <c:choose>
                <c:when test="${not empty sessionScope.usuarioLogado}">
                    <div class="detalhes-acoes">
                        ...
                        <%-- Adicionar / remover da ColeÃ§Ã£o --%>
                        <c:choose>
                            <c:when test="${estaNaColecao}">
                                <form action="${pageContext.request.contextPath}/colecao/adicionar" method="POST">
                                    ...
                                    <button type="submit" class="btn-acao active">
                                        <i class="fa-solid fa-check"></i> Na sua coleÃ§Ã£o
                                    </button>
                                </form>
                            </c:when>
                            <c:otherwise>
                                <form action="${pageContext.request.contextPath}/colecao/adicionar" method="POST">
                                    ...
                                    <button type="submit" class="btn-acao">
                                        <i class="fa-solid fa-plus"></i> Adicionar Ã  ColeÃ§Ã£o
                                    </button>
                                </form>
                            </c:otherwise>
                        </c:choose>
                ...
                <c:otherwise>
                    <div class="alert alert-error" style="max-width: 500px;">
                        <a href="${pageContext.request.contextPath}/login.jsp" style="color: var(--primary-accent);">FaÃ§a login</a>
                        para adicionar este disco Ã  sua coleÃ§Ã£o ou aos favoritos.
                    </div>
                </c:otherwise>
            </c:choose>

            <%-- Tracklist (placeholder atÃ© existir tabela faixa no banco) --%>
            <div class="tracklist">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <p style="color: var(--secondary-text); font-size: 0.85rem; padding: 0.5rem 0;">
                            A tracklist deste disco ainda nÃ£o foi cadastrada.
                        </p>
                    </c:otherwise>
                </c:choose>
            </div>
    ...
    <c:if test="${not empty sessionScope.usuarioLogado}">
        <section class="form-avaliar">
            <h3>
                <c:choose>
                    <c:when test="${not empty notaUsuario}">Atualizar minha avaliaÃ§Ã£o</c:when>
                    <c:otherwise>Deixar minha avaliaÃ§Ã£o</c:otherwise>
                </c:choose>
            </h3>
            ...
            <form action="${pageContext.request.contextPath}/avaliar-disco" method="POST">
                ...
                <div class="star-input">
                    ...
                    <label for="nota5">â˜…</label>
                    <input type="radio" id="nota4" name="nota" value="4" <c:if test="${notaUsuario == 4}">checked</c:if>>
                    <label for="nota4">â˜…</label>
                    <input type="radio" id="nota3" name="nota" value="3" <c:if test="${notaUsuario == 3}">checked</c:if>>
                    <label for="nota3">â˜…</label>
                    <input type="radio" id="nota2" name="nota" value="2" <c:if test="${notaUsuario == 2}">checked</c:if>>
                    <label for="nota2">â˜…</label>
                    <input type="radio" id="nota1" name="nota" value="1" <c:if test="${notaUsuario == 1}">checked</c:if>>
                    <label for="nota1">â˜…</label>
                </div>

                <div class="form-group">
                    <label for="comentario">ComentÃ¡rio (opcional)</label>
                    <textarea id="comentario" name="comentario" placeholder="O que vocÃª achou deste disco?"></textarea>
                </div>

                <button type="submit" class="btn-primary" style="max-width: 200px;">Salvar avaliaÃ§Ã£o</button>
            </form>
        </section>
    </c:if>

    <%-- ====================================================================
         Reviews / comentÃ¡rios do disco
         ==================================================================== --%>
    <section>
        <h2 class="section-heading" style="text-align: left; margin-top: 2rem;">
            <i class="fa-regular fa-comments"></i> ComentÃ¡rios
            <span class="count">${estatistica.total}</span>
        </h2>

        <c:choose>
            <c:when test="${empty reviews}">
                <p class="empty-state">Ainda nÃ£o hÃ¡ comentÃ¡rios sobre este disco. Seja o primeiro!</p>
            </c:when>
            <c:otherwise>
